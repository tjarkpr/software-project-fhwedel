...
import pickle
#KONSTANTEN
CROP_BOX = (15,15,45,45)
CROP_BOX_INDIRECT_SIZE = 30
CROP_PIX_SIZE = 60
CROP_H_SIZE = 32
CROP_V_SIZE = 18
TRAIN_SCENES = 65
...
trainImgIndirectSize = CROP_BOX_INDIRECT_SIZE * CROP_BOX_INDIRECT_SIZE * COLOR_CHANS
...
for i in range(0, TRAIN_SCENES + 1):
    ...
    for v in range(0, CROP_V_SIZE):
        for h in range(0, CROP_H_SIZE):
            ...
            croppedIndirectImgHint = croppedIndirectImg.crop(CROP_BOX)
            croppedDirectImgHint = croppedDirectImg.crop(CROP_BOX)
            Y[trainSetCounter] = (np.array(croppedIndirectImg).astype(np.int32) - np.array(croppedDirectImg).astype(np.int32)).flatten()
            X[trainSetCounter] = np.concatenate((np.array(croppedDirectImg).flatten(),(np.array(croppedIndirectImgHint).astype(np.int32) - np.array(croppedDirectImgHint).astype(np.int32)).flatten()))
