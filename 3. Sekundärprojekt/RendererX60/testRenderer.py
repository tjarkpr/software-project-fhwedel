...
testImgIndirectSize = CROP_BOX_INDIRECT_SIZE * CROP_BOX_INDIRECT_SIZE * COLOR_CHANS
...
for v in range(0, CROP_V_SIZE):
    for h in range(0, CROP_H_SIZE):
        ...
        X[testSetCounter] = np.concatenate((np.array(croppedDirectImg).flatten(),(np.array(croppedIndirectImgHint).astype(np.int32) - np.array(croppedDirectImgHint).astype(np.int32)).flatten()))
